---
title: "创建 SMR 集群"
description: 本小节主要介绍如何快速创建 SMR 集群实例。 
keywords: SMR 实例, 创建集群,创建实例
weight: 10
collapsible: false
draft: false
---

通过 AppCenter 集群管理控制台，您可以快速创建 SMR 集群。

本小节主要介绍如何快速创建 SMR 集群。
...
1. 登录管理控制台。
2. 选择**产品与服务** > **大数据服务** > **大数据基础服务 SMR**，进入 SMR 集群管理页面。
...
6. 确认配置和费用信息无误后，点击**提交**，创建集群。
   ...
   集群创建成功后，可在**集群管理**页面，查看和管理 SMR 集群。
...
![HDFS 主节点设置](../../_images/smr_step_2.png)
...
![网络设置](../../_images/smr_step_6.png)

### 依赖服务设置

选择依赖服务（External Service）后，可以将依赖服务所有节点加入 SMR 所有节点的 hosts 文件中。 HBase、Hadoop 或者 Spark 集成的场景经常会有这种需求，选定依赖 HBase 集群即可自动添加 hosts ，无需手动添加。

选择已创建的应用服务。

![依赖服务](../../_images/smr_step_7.png)

### 服务环境参数设置

集群环境参数配置，可修改集群默认配置。或集群创建成功后，在集群详情页面**配置参数**页签修改参数。

![服务环境参数](../../_images/smr_step_8.png)

### 用户协议

阅读**云平台 AppCenter 用户协议**，并勾选用户协议。

![用户协议](../../_images/smr_step_9.png)
