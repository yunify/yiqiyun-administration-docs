...
|   存储类型  |  选择集群节点存储磁盘类型。可选择`基础型`、`SSD 企业型`和`企业分布式 SAN(NeonSAN)`。<li>不同类型可选**磁盘大小**不同。   |
...
|   资源类型 |  选择云服务器类型。可选择`基础型`和`企业型  e2`。<li>不同类型可选 CPU 和内存不同。| 
|   存储类型  |  选择集群节点存储磁盘类型。可选择`基础型`、`SSD 企业型`和`企业分布式 SAN(NeonSAN)`。<li>不同类型可选**磁盘大小**不同。<span style="display: block; background-color: #D8ECDE; padding: 10px 24px; margin: 10px 0; border-left: 3px solid #00a971;"><b>注意</b>:<li>只有选择部署在**区域**时，才可以选择`企业级分布式 SAN (NeonSAN)`。</li></span>     |
